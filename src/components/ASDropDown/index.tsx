...
const ASDropDown: React.FC<ASDropDownProps> = (props: ASDropDownProps) => {
  ...
  const filterEmptyData = options
    ...
    : [];
  const filteredData = filterEmptyData
    ? searchValue
      ? filterEmptyData.filter((item) => {
  ...
  Object.keys(flatttenedContainerStyle || {}).forEach((key) => {
    ...
    } else if (!dropdownExcludedKeys.includes(key)) {
      ...
      borderColor:
        meta?.error && meta?.touched
          ...
          : isFocus
            ...
            : dropdownOnlyStyle?.borderColor || 'transparent',
    },
    {
      paddingTop:
  ...
  const flattenedIconStyles = StyleSheet.flatten(iconStyles) || {};
  const iconColorFromStyles =
    (
      flattenedIconStyles as ImageStyle & {
  ...
  const dropdownContent = !isMultiChoices ? (
